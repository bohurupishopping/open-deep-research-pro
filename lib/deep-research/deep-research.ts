...
export async function deepResearch({
  ...
  const tavilyClient = getTavily(firecrawlKey);
  ...
  for (const serpQuery of serpQueries) {
    try {
      ...
      const searchResults = await tavilyClient.search(serpQuery, {
      ...
      await logProgress(
        formatProgress.found(searchResults.results.length, serpQuery),
        onProgress,
      );

      if (searchResults.results.length > 0) {
        await logProgress(
          formatProgress.ran(serpQuery, searchResults.results.length),
          onProgress,
        );
        ...
        results.push({
          ...
          visitedUrls: searchResults.results
            .map(r => r.url)
