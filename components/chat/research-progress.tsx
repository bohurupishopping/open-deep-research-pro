...
export function ResearchProgress({
  ...
  useEffect(() => {
    if (!isLoading && progress.length > 0) {
      ...
      const isComplete = lastStep?.type === 'report' && 
        !lastStep.content.includes('Generating') && 
        !lastStep.content.includes('Synthesizing');
  ...
  // Hide the component when research is complete
