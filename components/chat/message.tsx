    ...
    >
      <div
        className={`flex gap-4 ${
          message.role === 'user'
            ? 'bg-foreground/5 py-3 px-6 rounded-2xl text-[15px] w-fit ml-auto max-w-[90%] font-medium'
            : 'w-full'
        }`}
      >
        <div
          className={`flex-1 ${
            message.role === 'assistant'
              ? 'prose prose-lg dark:prose-invert max-w-none [&>div>div]:text-base [&_p]:text-[15px] [&_li]:text-[15px] [&_h1]:text-2xl [&_h2]:text-xl [&_h3]:text-lg'
              : ''
          }`}
        >
          {message.role === 'assistant' ? (
